...
// Avrogo creates a struct automagically after defining the json structure in the client.asvc file, we can get the json on AkHQ
...
func produce() {
	...
	payload, err := ser.Serialize(topic, &Client{Name: "Test", Country: "Brazil", City: "Blumena"})
	if err != nil {
		panic(err)
	}
